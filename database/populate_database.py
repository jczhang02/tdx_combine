...
def stocks2database(session: Session, df: pd.DataFrame) -> None:
    for _, row in df.iterrows():
        ...
        stock = Stock(code=row["code"], blocks=blocks)
...
def infoharbor2database(session: Session, df: pd.DataFrame) -> None:
    for _, row in df.iterrows():
        stocks_index: List[str] = row["stocks"]
        block_code: str = row["code"]
        if not stocks_index:
            continue

        for stock_index in stocks_index:
            if not session.query(Stock).filter(Stock.code == stock_index).one_or_none():
                stock = Stock(code=stock_index)
                session.add(stock)

        session.flush()

        stocks_to_add = session.query(Stock).filter(Stock.code.in_(stocks_index)).all()
        block = session.query(Block).filter(Block.code == block_code).one_or_none()

        if block and stocks_to_add:
            block.stocks.extend(stocks_to_add)
